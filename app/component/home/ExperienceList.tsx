...
const ExperienceCardBody = ({ item }: { item: ExperienceItem }) => (
  <>
    ...
    <div className="flex gap-2 text-sm mb-4">
      <p>
        {item.start_mount} {item.start_year}
      </p>
      <p>-</p>
      <p>
        {item.end_mount} {item.end_year}
      </p>
    </div>
    <div className="mb-4">
      ...
      {items.map((e, index) => (
        <li key={index} className="mb-12">
          <div className="group relative transition-all lg:hover:!opacity-100 lg:group-hover/list:opacity-50">
            ...
            {e.website ? (
              <Link href={e.website} className="z-10 block" target="_blank">
                <ExperienceCardBody item={e} />
              </Link>
            ) : (
              <div className="z-10">
                <ExperienceCardBody item={e} />
